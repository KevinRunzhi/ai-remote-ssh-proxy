...
- **Blocking**: GitHub's native issue dependencies. Where dependencies aren't available, use a `Blocked by: #<n>, #<n>` line at the top of the child body.
- **Frontier query**: list the map's open children, then drop assigned tickets and tickets with open blockers; first in map order wins.
